...
- 保持低层启发式边（co-occurs, appears_in, located_in）作为“硬事实”，LLM 生成的边作为“高阶推理”层。
 ...
 - 语义标注职责划分（检测 vs VLM）：  
   - 检测/追踪（如 YOLO）主要承担**实例几何与筛选**：给出人/物体的 bbox、轨迹与关键帧，判断“这段是否值得进一步语义化”；  
   - 具体“场景/物体/行为/关系”的语义识别，逐步由 VLM 统一生成，通过严格的 JSON Schema 约束输出：  
     - 例如 `{"scene": "...", "objects": [...], "actions": [...], "interactions": [...]}；`  
   - 在图中：  

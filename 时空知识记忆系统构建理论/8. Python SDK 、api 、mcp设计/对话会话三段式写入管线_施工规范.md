...
- **默认 best‑effort（生产）**：字段校验失败时进行**纠偏**（非法枚举→默认值、非法 span→忽略），不让整批失败，保证管线可用性。  
- **strict 模式（验收/benchmark）**：任一字段校验失败即 Stage2 失败 → 进入重试（记录错误码与 next_retry_at）。  
- Stage2 成功但 `requires_confirmation=true`：允许进入 Stage3，但写入的 Knowledge/Note 必须标注“待确认”（不得当硬事实传播）。

> 工程实现建议：通过 `client_meta.stage2_strict=true` 打开 strict；默认关闭。

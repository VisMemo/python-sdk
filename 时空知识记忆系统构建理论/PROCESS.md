...
## 9. 本周期新增工作内容（Phase 0/1 落地计划，2025-12-25）

1. 新增 Phase 0/1 可执行计划文档：`docs/时空知识记忆系统构建理论/7. 记忆检索与推理落地/PHASE0_1_PLAN.md`。  
2. 明确 Phase 0（/ingest → 图可查 → Demo 可见）与 Phase 1（L1/L2 可答）的执行顺序、DoD 与查询模板。  
3. 将 L1/L2 关键问题映射到具体 Graph API 查询路径，确保后续 UI 与测试可直接复用。  

## 10. 测试与验证（Phase 0/1 文档）

本周期为计划文档与执行清单更新，未运行测试。

## 11. 结果

Phase 0/1 已具备可执行的落地清单与验证路径，可进入实际执行阶段。
